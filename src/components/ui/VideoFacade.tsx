...
interface VideoFacadeProps {
  /** YouTube video id, e.g. "WpxuRP-p6CU" */
  videoId: string
  title: string
  channel: string
  /** poster image (JPG path under public/) */
  poster: string
  posterAlt: string
  className?: string
}
 ...
 *
 * Nothing loads from Google until the visitor presses play — the facade is a
 * plain poster image + button (no third-party JS on page load, aligned with
 * the site's click-to-load policy for external embeds). After the click the
 * video mounts via the privacy-enhanced youtube-nocookie.com domain.
 */
export default function VideoFacade({
  ...
  return (
    <div className={`group relative overflow-hidden ${className ?? ''}`}>
      ...
      >
        <picture>
          <source srcSet={`${poster.replace(/\.jpg$/, '')}.webp`} type="image/webp" />
          <img
            src={poster}
            alt={posterAlt}
        ...
        <span className="absolute inset-0 flex items-center justify-center">
          <span className="flex h-16 w-16 items-center justify-center rounded-full bg-gold-500 shadow-lg ring-4 ring-white/40 transition-transform duration-300 group-hover:scale-110">
